...
const RESULT_STYLE = {
  ...
  PENDING: { bg: 'rgba(249,115,22,0.08)',  border: 'rgba(249,115,22,0.2)',   text: '#f97316' },
  SKIPPED: { bg: 'rgba(139,148,158,0.08)', border: 'rgba(139,148,158,0.2)', text: '#8b949e' },
};

const CONF_COLORS = { HIGH: '#00c853', MEDIUM: '#f59e0b', LOW: '#8b949e' };
const REC_COLORS  = {
...
export default function HittingHistoryLog({ records = [], skippedRecords = [], loading = false, onDeleteRecord }) {
  ...
  async function handleDelete(date, batter_name) {
    try {
      await api.hittingDeleteLine({ date, batter_name });
      onDeleteRecord?.(date, batter_name);
    } catch (e) {
  ...
  if (loading) return (
    <div style={{ color: 'var(--text-muted)', padding: '2rem', textAlign: 'center' }}>Loading history...</div>
  );

  if (records.length === 0 && skippedRecords.length === 0) return (
    <div style={{ color: 'var(--text-muted)', padding: '2rem', textAlign: 'center' }}>
      No hitting picks logged yet. Picks are auto-logged each day when the slate loads.
    </div>
  );

  const skippedResolved = skippedRecords.filter(r => r.actual_hits != null && r.predicted_hits != null);
  const avgMiss = skippedResolved.length
    ? (skippedResolved.reduce((s, r) => s + (r.miss ?? 0), 0) / skippedResolved.length).toFixed(2)
    : null;
  const bigMisses = skippedResolved.filter(r => Math.abs(r.miss ?? 0) >= 2).length;
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
        {[
          ...
          const s = label === 'PENDING' ? RESULT_STYLE.PENDING : (RESULT_STYLE[label] ?? RESULT_STYLE.PENDING);
        ...
        <div style={{ marginLeft: 'auto', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          {!isSkipped ? (
            <>
              ...
              {pending.length > 0 && <span style={{ marginLeft: 6, color: ACCENT }}>· {pending.length} pending</span>}
              {(wins + losses) > 0 && (
            ...
            <span style={{ display: 'flex', gap: '1rem' }}>
              {avgMiss != null && (
                <span>Avg miss: <span style={{ fontWeight: 700, color: parseFloat(avgMiss) > 0 ? 'var(--accent-green)' : 'var(--accent-red)' }}>
                  {parseFloat(avgMiss) > 0 ? '+' : ''}{avgMiss}H
                </span></span>
              )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }}>
                ...
                <ColHeader label="Batter" k="batter_name" />
                <ColHeader label={isSkipped ? 'Would Bet' : 'Rec'} k="recommendation" />
                <ColHeader label="Line"   k="line" />
                <ColHeader label="Pred H" k="predicted_hits" />
                <ColHeader label="Actual" k="actual_hits" />
            ...
            <tbody>
              ...
              {sorted.map((row, i) => {
                ...
                const missColor = miss == null ? 'var(--text-muted)'
                  : Math.abs(miss) >= 2 ? '#f59e0b'
                  : miss > 0 ? 'var(--accent-green)' : 'var(--accent-red)';

                return (
                  <motion.tr key={`${row.date}-${row.batter_name}-${isSkipped}`}
                  ...
                  >
                    ...
                    <td style={{ padding: '0.6rem 0.75rem', fontWeight: 600, fontSize: '0.84rem', color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>
                      {row.batter_name}
                    </td>
                    ...
                    {isSkipped ? (
                      ...
                      <>
                        ...
                        <td style={{ padding: '0.6rem 0.5rem', textAlign: 'center' }}>
                          {isPending && (
                            <button
                              onClick={() => handleDelete(row.date, row.batter_name)}
                              title="Remove pick"
